...
// this takes in form data instead of a regular req
const post = async function (req, res) {
    try {
        let date = new Date();
        let dogData = JSON.parse(req.body.data);
        dogData.image = req.body.image;
        dogData.registered = date.toISOString();
        // validate the dog object
        const { error } = validateDog(dogData);
        if (error) {
...
const dogUpdate = async (req, res) => {
    try {
        ...
        dogData.registered = date.toISOString();
        // update mongo
        const updatedDog = await updateDog(req.params.id, dogData);
